...
// Interactive "Дыбыс картасы" for g0ArticulationMap.
// The modal markup stays untouched; this file only re-binds the map behavior.
...
function renderArticulationMap() {
  ...
  ARTIC_RING_2.forEach((letter) => {
    ...
    svg.appendChild(createArticulationNode({
      ...
      onClick: () => {
        if (isVisible) handleRing2Click(letter);
      },
    }));
  });
  ...
  ARTIC_RING_3.forEach((letter) => {
    ...
    svg.appendChild(createArticulationNode({
      ...
      onClick: () => {
        if (isVisible) handleRing3Click(letter);
      },
    }));
  });
