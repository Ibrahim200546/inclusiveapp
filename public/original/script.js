...
function sbStopRemoteAudio() {
  if (typeof window.appTts?.stop === 'function') {
    window.appTts.stop();
  }
}

async function sbSpeakWithYandex(text) {
  if (typeof window.appTts?.speakText !== 'function') {
    return false;
  }
  ...
  try {
    ...
    if (!result?.ok) {
      console.warn('Sentence Yandex TTS failed:', result);
      return false;
    }

    return true;
  } catch (error) {
    console.warn('Sentence Yandex TTS failed:', error);
    return false;
  }
}
...
async function sbPlaySentence() {
  ...
  } else if (typeof window.appTts?.stop === 'function') {
    window.appTts.stop();
  }

  const playedWithYandex = await sbSpeakWithYandex(text);
  if (!playedWithYandex) {
    console.warn('Sentence TTS is unavailable for the current sentence.');
  }
}
