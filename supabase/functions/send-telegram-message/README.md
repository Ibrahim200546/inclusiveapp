...
- Exposes `GET /thread` for restoring the latest user thread
...
- `CONTACT_WEBHOOK_SECRET`
  If omitted, the function accepts the Supabase anon key for database webhook auth.
...
4. Create the database webhook:
   ...
   - Headers:
     ...
     - `Authorization: Bearer <SUPABASE_ANON_KEY or CONTACT_WEBHOOK_SECRET>`
